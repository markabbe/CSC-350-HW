...
from oswegonlp.parsing import ParserState, TransitionParser, DepGraphEdge, train
from oswegonlp.utils import DummyCombiner, DummyActionChooser, DummyWordEmbedding, DummyFeatureExtractor, initialize_with_pretrained, build_suff_to_ix
from oswegonlp.data_tools import Dataset
from oswegonlp.constants import *
from oswegonlp.evaluation import compute_metric, fscore, dependency_graph_from_oracle
from oswegonlp.feat_extractors import SimpleFeatureExtractor
from oswegonlp.neural_net import FFActionChooser, FFCombiner, VanillaWordEmbedding, BiLSTMWordEmbedding, LSTMCombiner, LSTMActionChooser, SuffixAndWordEmbedding
...
def check_tensor_correctness(pairs):
    for pair in pairs:
        for pred, true in zip(pair[0], pair[1]):
            assert pred == approx(true, abs=1e-4)
...
def test_get_arc_components_d1_1a():
    ...
    assert (head.headword == "dog")
    assert(head.headword_pos ==  4)
    assert(head.embedding == "dog-EMBEDDING")
    assert(mod.headword == "the")
    assert(mod.headword_pos == 3)
    assert(mod.embedding == "the-EMBEDDING")

    head, mod = state._get_arc_components(Actions.ARC_R)
    assert(head.headword == "saw")
    assert(head.headword_pos == 2)
    assert (head.embedding == "saw-EMBEDDING")
    assert (mod.headword == "with")
    assert (mod.headword_pos == 5)
    assert (mod.embedding == "with-EMBEDDING")

    head, mod = state._get_arc_components(Actions.ARC_L)
    assert (head.headword == "the")
    assert (head.headword_pos == 6)
    assert (head.embedding == "the-EMBEDDING")
    assert (mod.headword == "man")
    assert (mod.headword_pos == 1)
    assert (mod.embedding == "man-EMBEDDING")
...
def test_create_arc_d1_1b():
    ...
    assert (left_arc[0][0] == "man")
    assert (left_arc[0][1] == 1)
    # dependent should be "The"
    assert (left_arc[1][0] == "The")
    assert (left_arc[1][1] == 0)

    # check the top of the input buffer to make sure combination was right
    assert (state.input_buffer[0].headword == "man")
    assert (state.input_buffer[0].headword_pos == 1)
    assert (state.input_buffer[0].embedding == "man-EMBEDDING")
    ...
    assert (right_arc[0][0] == "saw")
    assert (right_arc[0][1] == 2)
    # dependent should be "the"
    assert (right_arc[1][0] == "the")
    assert (right_arc[1][1] == 3)

    assert (state.input_buffer[0].headword == "saw")
    assert (state.input_buffer[0].headword_pos == 2)
    assert (state.input_buffer[0].embedding == "saw-EMBEDDING")
...
def test_validate_action_d1_3():
    ...
    assert (valid_action == Actions.SHIFT)
    state.shift()
    state.arc_left()
    state.shift()
    state.arc_left()
    ...
    valid_action = state._validate_action(act_to_do)
    assert (valid_action == Actions.ARC_R)
...
def test_feature_extraction_d2_2():
    ...
    true = ([-0.7121446132659912, 0.30372199416160583, -0.777314305305481, -0.25145524740219116],
            [-0.6969724297523499, -1.1607614755630493, 0.6995424032211304, 0.1990816295146942],
            [0.199056506156921, 0.04570277780294418, 0.15295691788196564, -0.47567880153656006])
...
def test_parse_logic_d3_1():
    ...
    true_out = [-0.9270416498184204, -1.2309693098068237, -1.1638933420181274]
...
def test_dev_d3_2_english():
    preds = open(EN_D3_2_DEV_FILENAME, 'r', errors='ignore')
    gold = open(EN_DEV_GOLD, 'r', errors='ignore')
...
def test_dev_d3_3_norwegian():
    preds = open(NR_D3_3_DEV_FILENAME, errors='ignore')
    gold = open(NR_DEV_GOLD, errors='ignore')
...
def test_lstm_combiner_d4_4():
    ...
    # Do the combination a few times to make sure implemented the sequential part right
    combined = combiner(head_feat, modifier_feat)
    combined = combiner(head_feat, modifier_feat)
    combined = combiner(head_feat, modifier_feat)
...
def test_lstm_action_chooser_d4_5():
    ...
    # Run the action chooser a few times to make sure implemented the sequential part right
