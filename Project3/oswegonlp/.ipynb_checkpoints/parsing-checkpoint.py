...
from oswegonlp.constants import Actions, NULL_STACK_TOK, END_OF_INPUT_TOK, HAVE_CUDA, ROOT_TOK
...
class ParserState:
    ...
    def done_parsing(self):
        ...
        # Check if there is exactly one element in both the input buffer and the stack
    ...
    def _create_arc(self, head, modifier):
        ...
        # Combine the embeddings of head and modifier
        combined_embedding = self.combiner(head[2], modifier[2])
        # Insert a new stack entry at the beginning of the input buffer
        self.input_buffer.insert(0, StackEntry(head[0], head[1], combined_embedding))
        # Create a new dependency graph edge
        updated_edge = DepGraphEdge((head[0], head[1]), (modifier[0], modifier[1]))
        # Return the new dependency graph edge
        return updated_edge
    ...
    def _validate_action(self, action_to_do):
        ...
        if action_to_do == Actions.SHIFT:
            # Shift condition: at least two items in the input buffer or empty stack
            if self.input_buffer_len() <= 2 and self.stack_len() >= 1:
                return Actions.ARC_R
            else:
                return action_to_do
        elif action_to_do == Actions.ARC_R:
            # ARC_R condition: cannot perform if the stack is empty
            if self.stack_len() < 1:
                return Actions.SHIFT
            else:
                return action_to_do
        else:
            # Initialize a local modifier
            local_modifier = 2
            if self.stack_len() >= 1 and (self.stack_peek_n(1)[0].headword != ROOT_TOK):
                # Double the modifier if the top of the stack is not ROOT_TOK
                local_modifier = local_modifier * 2
                return action_to_do
            elif self.stack_peek_n(1)[0].headword == ROOT_TOK:
                # Increment the modifier if the top of the stack is ROOT_TOK
                local_modifier = local_modifier + 1
                if self.input_buffer_len() > 2:
                    return Actions.SHIFT
                else:
                    return Actions.ARC_R
            else:
                # Decrement the modifier otherwise
                local_modifier = local_modifier - 2
                return Actions.SHIFT
    ...
    def _get_arc_components(self, action):
        ...
        if action == Actions.ARC_L:
            # Popping the first element from the input buffer
            current_head = self.input_buffer.pop(0)
            # Popping the top element from the stack
            current_modifier = self.stack.pop()
        else:
            # Popping the top element from the stack
            current_head = self.stack.pop()
            # Popping the first element from the input buffer
            current_modifier = self.input_buffer.pop(0)
        
        return current_head, current_modifier
...
class TransitionParser(nn.Module):
    ...
    def forward(self, sentence, actions=None):
        ...
        The boilerplate at the beginning initializes a valid
        ...
        if actions is not None:
            action_queue = deque([Actions.action_to_ix[a] for a in actions])
            have_gold_actions = True
        else:
            have_gold_actions = False
    
        # Loop until all parsing actions are completed
        while not parser_state.done_parsing():
            # Extract features from the current parser state
            extracted_features = self.feature_extractor.get_features(parser_state)
        
            # Process actions depending on whether gold standard actions are provided
            if have_gold_actions:
                # Get the next action to perform from the queue
                next_action = action_queue.popleft()
                # Compute log probabilities of actions based on features
                action_log_probs = self.action_chooser(extracted_features)
                ...
                # Store the log probabilities
                outputs.append(action_log_probs)
                # Choose the action with the highest log probability
                next_action = utils.argmax(action_log_probs.data)
        
            # Validate the chosen action
            valid_action = parser_state._validate_action(next_action)
            # Record the validated action
            actions_done.append(valid_action)
        
            # Perform the validated action
            if valid_action == Actions.SHIFT:
                parser_state.shift()
            elif valid_action in (Actions.ARC_L, Actions.ARC_R):
                # Perform arc operation and get the dependency edge
                dependency_edge = parser_state._arc(valid_action)
                # Add the dependency edge to the graph
                dep_graph.add(dependency_edge)
        
        # Return the list of outputs, the dependency graph, and the list of actions performed
