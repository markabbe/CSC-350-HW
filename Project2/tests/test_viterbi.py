...
def test_compute_hmm_weights_variables():
    ...
    assert (emission_probs[0][0].data.numpy() == -np.inf)
    assert (emission_probs[0][1].data.numpy() == -1)
    assert (emission_probs[0][2].data.numpy() == -11)
    assert (emission_probs[2][1].data.numpy() == -3)
    assert (emission_probs[2][2].data.numpy() == -4)
    
    assert (tag_transition_probs[0][0].data.numpy() == -np.inf)
    assert (tag_transition_probs[1][1].data.numpy() == -5)
    assert (tag_transition_probs[2][2].data.numpy() == -3)
    assert (tag_transition_probs[2][3].data.numpy() == -np.inf)
    assert (tag_transition_probs[3][3].data.numpy() == -np.inf)
...
def test_viterbi_step_init():
    ...
    assert (viterbivars[1].data.numpy() == -2)
    assert (viterbivars[2].data.numpy() == -13)
    assert (bptrs[1] == 0)
    assert (bptrs[2] == 0)
    assert (bptrs[3] == 0)
    ...
    assert (viterbivars[1].data.numpy() == -10)
    assert (viterbivars[2].data.numpy() == -6)
    assert (bptrs[1] == 1)
    assert (bptrs[2] == 1)
    assert (bptrs[3] == 0)
...
def test_trellis_score():
    ...
    assert (path_score.data.numpy() == -17)
    ...
    assert (path_score.data.numpy() == -25.)
...
def test_build_trellis():
    ...
    assert (best_path == ['NOUN', 'VERB', 'VERB', 'NOUN'])
    ...
    assert (best_path == ['NOUN', 'VERB', 'VERB', 'NOUN', 'VERB', 'NOUN'])
